...
if(!function_exists('woocommerce_inactive_notice_stripe')) {
	function woocommerce_inactive_notice_stripe() {
		...
		<p><?php printf( __( '%sWCMp Vendor Stock Alert is inactive.%s The %sWooCommerce plugin%s must be active for the WCMp Vendor Stock Alert to work. Please %sinstall & activate WooCommerce%s', WCMp_STRIPE_GATEWAY_TEXT_DOMAIN ), '<strong>', '</strong>', '<a target="_blank" href="http://wordpress.org/extend/plugins/woocommerce/">', '</a>', '<a href="' . admin_url( 'plugin-install.php?tab=search&s=woocommerce' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
		</div>
		<?php
	}
}
